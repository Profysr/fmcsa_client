...
(() => {
  ...
  async function apiFetch(url, options = {}) {
    ...
    const expiry = parseInt(
      sessionStorage.getItem(sessionKeys.tokenExpiry),
      10
    );
  ...
  function getFieldValue(el, labelText) {
  ...
  localStorage.getItem(STORAGE.runFlag);
  localStorage.getItem(STORAGE.rangeSetFlag); // ./src/utils/fingerprint.js
  ...
  function showRangeForm() {
    ...
    document.getElementById("startScan").addEventListener("click", async () => {
      ...
      count > 500 && alert("Count must be less than 500");
  ...
  //   });
  // }
  async function showTokenRequestForm() {
    const overlay = document.createElement("div");
    overlay.style = `
    position: fixed; top: 0; left: 0; width: 100vw; height: 100vh;
    background: rgba(0,0,0,0.5); z-index: 9998;
  ...
  async function downloadCSVFromServer(filename = "data.csv") {
    try {
  ...
  const src_rangeSet = localStorage.getItem(STORAGE.rangeSetFlag) === "true";
  const isTokenValid = Object.keys(sessionKeys).every(
    (key) => !!sessionStorage.getItem(key)
  );
  function handleSnapshotPage() {
    if (src_shouldRun && !isTokenValid) {
      showTokenRequestForm();
      return;
    }
    if (src_shouldRun && isTokenValid && !src_rangeSet) {
      showRangeForm();
      return;
    }
    if (src_shouldRun && isTokenValid && src_rangeSet) {
      ...
      if (current <= end) setTimeout(() => submitQuery(current), 2e3);
      else {
  ...
  async function handleQueryPage() {
    ...
    if (current <= end) setTimeout(() => submitQuery(current), 1500);
    else {
      downloadCSVFromServer(
  ...
  location.href.includes("CompanySnapshot.aspx")
    ? handleSnapshotPage()
    : location.href.includes("query.asp") &&
      src_shouldRun &&
      src_rangeSet &&
      handleQueryPage();
})();
