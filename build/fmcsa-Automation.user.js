...
function showRangeForm(){const overlay=document.createElement("div");overlay.style=`
    ...
    `;document.body.appendChild(box);const startBtn=document.getElementById("startScan");const rangeFormAction=()=>{const start=parseInt(document.getElementById("startNum").value);const end=parseInt(document.getElementById("endNum").value);if(isNaN(start)||isNaN(end)||start>end){alert("❌ Invalid range.");return}const count=end-start+1;count>500&&alert("Count must be less than 500");localStorage.setItem(STORAGE.rangeKey,JSON.stringify({start,end}));saveCurrent(start);localStorage.setItem(STORAGE.rangeSetFlag,"true");location.reload()};startBtn.addEventListener("click",rangeFormAction)}
...
async function downloadCSVFromServer(filename="data.csv"){try{await new Promise((resolve=>setTimeout(resolve,2500)));
// alert("Come to download");
const res=await apiFetch("/api/download-csv");console.log("res: ",res);if(!res.ok)throw new Error(res);const blob=await res.blob();const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=filename;document.body.appendChild(a);a.click();document.body.removeChild(a)}catch(err){alert(`CSV download failed:, ${err}`)}}// ./src/index.js
addFloatingToolbar();const src_shouldRun=localStorage.getItem(STORAGE.runFlag)==="true";const src_rangeSet=localStorage.getItem(STORAGE.rangeSetFlag)==="true";const isTokenValid=validateSessionStorage();async function handleSnapshotPage(){if(src_shouldRun&&!isTokenValid){showTokenRequestForm();return}if(src_shouldRun&&isTokenValid&&!src_rangeSet){showRangeForm();return}if(src_shouldRun&&isTokenValid&&src_rangeSet){const{start,end}=getRange();const current=getCurrent();if(current<=end)submitQuery(current);else{await downloadCSVFromServer(`Record of ${start}-${end} at ${(new Date).toLocaleDateString()}.csv`);alert("✅ Completed MX/MC range.");clearAllStorage()}}}async function handleQueryPage(){const{start,end}=getRange();let current=getCurrent();const table=document.querySelector("table");if(!table)return;const text=table.innerText;if(text.includes("Record Inactive")||text.includes("Record Not Found")){
// alert(`❌ MX ${current} is inactive.`);
current++;saveCurrent(current);goToSnapshot()}else if(text.includes("USDOT INFORMATION")){const isValid=validateActiveTable(table);
// alert(`is valid: ${isValid}`);
if(isValid){const record={};requiredValues.forEach((key=>{record[key]=getFieldValue(table,key)||"NOT FOUND"}));try{const response=await apiFetch(`/api/validate-record`,{method:"POST",body:JSON.stringify({record})});await response.json()}catch(error){console.error(`❌ Error sending record: ${error}`)}}current++;saveCurrent(current)}if(current<=end)submitQuery(current);else{await downloadCSVFromServer(`Record of ${start}-${end} at ${(new Date).toLocaleTimeString()}.csv`);clearAllStorage();alert("✅ Finished checking all numbers.");goToSnapshot()}}location.href.includes("CompanySnapshot.aspx")?handleSnapshotPage():location.href.includes("query.asp")&&src_shouldRun&&src_rangeSet&&handleQueryPage()})();